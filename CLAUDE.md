...
### Turbopack キャッシュ起因のエラー

以下のようなエラーが出た場合は **即座に** `.next` を削除して再起動する：

- `Turbopack のキャッシュが古い package.json を保持しています`
- `Failed to resolve` / `Cannot find module` 系でパッケージは存在するのに解決できない
- `Module not found` が突然発生してコード変更では直らない

```bash
# web アプリの場合（ルートから）
pnpm dev:web:clean

# apps/web ディレクトリから直接の場合
pnpm dev:clean
```

dev サーバー起動中にこれらのエラーを検知したら、サーバーを停止して上記コマンドを実行すること。確認を取らずに実行してよい。
