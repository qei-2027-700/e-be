---
name: harness-lint
description: .Codex/ 配下のハーネス構成（skills・commands・rules・steering）を静的チェックするスキル。フロントマターの欠落・重複・不正値を検出して報告する。
model: haiku
---

`.Codex/` 配下のファイルを検査し、問題点を一覧で報告してください。

## チェック対象と観点

### 1. `.Codex/skills/*/SKILL.md` — スキルファイル
...
### 3. `.Codex/commands/*.md` — コマンドファイル

- フロントマターがある場合、`---` が正しく開閉されているか
- ファイルが空でないか

### 4. `.Codex/steering/issue-*.md` — ステアリングファイル
...
find .Codex/rules -name "*.md"
find .Codex/commands -name "*.md"
find .Codex/steering -name "issue-*.md"
```
